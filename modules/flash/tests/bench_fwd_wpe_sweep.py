...
Usage:
    ...
    ~/.venvs/theRock/bin/python \\
        modules/flash/tests/bench_fwd_wpe_sweep.py [--head_dims 256 512] [...]
...
# ROCM_PATH, via the project's own resolver. Without it flydsl fails at link
# time with a bare `error: lld invocation failed` that says nothing about the
# toolchain. Loaded by path so `python/` does not go on sys.path.
