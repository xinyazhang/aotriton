...
#ifndef AOTRITON_V2_API_SCHEMALESS_H
#define AOTRITON_V2_API_SCHEMALESS_H
...
// A borrowed, read-only view over a ';'-separated 'k=v' string (PLAN-PHASE2.md
// Task 2, Design B). Holds no storage: the backing text is the compiled-in
// packed_string, which has static storage duration. Never construct from a
// temporary std::string.
//
// Grammar (measured from the packed strings actually emitted):
//   section := pair (';' pair)*
//   pair    := key '=' value
