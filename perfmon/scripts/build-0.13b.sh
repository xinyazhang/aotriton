...
# Same grouping as 0.12.1b: gfx110x/gfx115x split, gfx1250 present.
# +asan variants also exist and are never selected.
case "${ARCH%%:*}" in
    ...
    *)
      echo "Error: AOTriton 0.13b publishes no kernel-image asset covering" >&2
      echo "       '${ARCH}'. See https://github.com/ROCm/aotriton/releases/tag/0.13b" >&2
      exit 1
      ;;
esac
ASSET="aotriton-0.13b-images-amd-${IMAGES_GROUP}.tar.gz"
...
TARBALL_NAME="$(fetch_release_asset "0.13b" "${ASSET}" "${IMAGES_DL_DIR}")"
