...
struct [[context_class_name]] {
    ...
    // IMPLEMENTS them (PLAN-PHASE2.md Task 6, out of scope for Tasks 1-5). Each
    ...
    // launch -- see PLAN-PHASE2.md Task 5).
    ...
    // cannot otherwise write through a const reference), then read by pointer
    // from the kernarg vector. Lifetime is the context's, which outlives
    // launch().
