...
def test_prime_set_size_at_max_seqlen_16384():
    ...
    # 8 hdims x 2 causal x 11 seqlens x 2 dtypes.
    ...
    assert len(entries) == 8 * 2 * 11 * 2 == 352


def test_prime_set_size_at_max_seqlen_4096():
    pd = _perfdesc()
    entries = list(pd.prime_entries('gfx950', 4096))
    # 8 hdims x 2 causal x 9 seqlens (16..4096; 8192 and 16384 excluded)
    # x 2 dtypes
    assert len(entries) == 8 * 2 * 9 * 2 == 288
